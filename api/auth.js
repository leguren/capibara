...
module.exports = async function handler(req, res) {
  ...
  if (sub === 'callback') {
    ...
    try {
      ...
      if (!tokenRes.ok) return res.redirect(302, '/login?error=token_exchange');
      const tokens = await tokenRes.json();
      const profileRes = await fetch(GOOGLE_USERINFO_URL, {
        headers: { Authorization: `Bearer ${tokens.access_token}` },
      });
      if (!profileRes.ok) return res.redirect(302, '/login?error=profile_fetch');
      const profile = await profileRes.json();
      const email = profile.email;
      if (!email) return res.redirect(302, '/login?error=no_email');
      const db = getDb();
      const existing = await db.execute({ sql: 'SELECT id, email, name, picture, role, tier FROM users WHERE email = ? LIMIT 1', args: [email] });
      let user;
      if (existing.rows.length) {
        user = existing.rows[0];
        await db.execute({ sql: 'UPDATE users SET last_login = ?, name = ?, picture = ? WHERE id = ?', args: [now(), profile.name || email, profile.picture || null, user.id] });
      } else {
        const newId = id('usr');
        await db.execute({ sql: `INSERT INTO users (id, email, name, picture, role, tier, created_at, last_login) VALUES (?, ?, ?, ?, 'user', NULL, ?, ?)`, args: [newId, email, profile.name || email, profile.picture || null, now(), now()] });
  ...
  if (sub === 'me') {
    ...
    try {
      await initSchema();
      const db = getDb();
      const result = await db.execute({ sql: 'SELECT id, email, name, picture, role, tier, created_at, last_login FROM users WHERE id = ? LIMIT 1', args: [session.userId] });
